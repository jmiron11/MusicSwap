...
var findMatch = function(username, socketid, artist1, artist2, artist3, io)
{
    var collection
    var db
            ...
            function processDocument(err, user){
                ...
                if(artistMatched != null)
                {
                    ...
                    addUsersToConnected(username, socketid, user.username, user.socketid, function(err){
                        io.sockets.socket(socketid).emit("match_found", user.username, artistMatched)
                        io.sockets.socket(user.socketid).emit("match_found")
                        callback(err)
                    })
                    callback(null);
                }
